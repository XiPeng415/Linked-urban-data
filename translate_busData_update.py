...
####
####
#### I make a mistake that confuse long and lat on x and y,
#### so I need to exchange here.
#### I change x_csv and y_csv
####
y_csvFile_name = r"C:\Users\User\Documents\GitHub\ShareFolder\vvsBusData\busData\x.csv.gz"
x_csvFile_name = r"C:\Users\User\Documents\GitHub\ShareFolder\vvsBusData\busData\y.csv.gz"
...
for stop in dict_stops:
    ...
    ttl_file.write(f"ex:stopID_{stop} a ex:Stop_VVS ;\n")
...
for route in dict_shape:
        ...
        for id, (x_num, y_num) in enumerate(zip(xList, yList)):
            ...
            ttl_file.write(f"ex:long {x_num} ;\n")
            ttl_file.write(f"ex:lat {y_num} .\n")
...
for key in dict_trip:
    ...
    if f<60593:
        stop_sequence_key = []
        for inside_key in dict_trip[key]:
            if isinstance(inside_key, int):
                stop_sequence_key.append(inside_key)
        ...
        for id in stop_sequence_key:
            ...
            if id_counter == len(stop_sequence_key):
                ttl_file.write(f"ex:{rank[id-1]}_ETA '{arrvTime}' ; \n ")
                ttl_file.write(f"ex:{rank[id-1]}_AT '{depTime}' ; \n ")
                ttl_file.write(f"ex:the_{rank[id-1]}_station ex:stopID_{stop_id} .\n") # Add new node called key_id   ############################################
            else:
                ttl_file.write(f"ex:{rank[id-1]}_ETA '{arrvTime}' ; \n ")
                ttl_file.write(f"ex:{rank[id-1]}_AT '{depTime}' ; \n ")
                ttl_file.write(f"ex:the_{rank[id-1]}_station ex:stopID_{stop_id} ;\n") # Add new node called key_id

           # elif id not in dict_trip[key].keys() and i + 3 == len(dict_trip[key]):
